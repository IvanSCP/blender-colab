...
This fork introduces **EEVEE render engine support via Vulkan** and **embedded script automation** while maintaining full backward compatibility with the original CYCLES workflow.

## Features

* **EEVEE Rendering**: Full support for EEVEE/EEVEE Next engine using **Vulkan GPU acceleration** (Blender 3.5+ recommended). No X11 or virtual display required.
...
### Render Settings
* `render_engine`: Choose between `CYCLES` (CUDA/OptiX) or `EEVEE` (Vulkan GPU).
...
3. **EEVEE & Vulkan**: This fork uses Vulkan for headless EEVEE rendering on NVIDIA GPUs (Tesla T4, etc.). Requires Blender 3.5 or newer for best compatibility. Older versions may fallback to CPU.
4. **GPU Access**: Do note that your access to GPU may be limited or blocked if you render for many hours.
5. This script is intended for those who have no access to high-end GPU for rendering. Please use them responsibly!
...
* `vkCreateInstance failed`: Ensure you have selected a GPU runtime (Runtime > Change runtime type > Hardware accelerator > GPU).
